...
# in both normal and cancer tissue 


# To subest the gene nemaes found above
top_genes <- top_15 %>%
  ungroup() %>%
  select(genename) %>%
  unique() %>%
...
```{r }
# how SFTPC gene expressed in all individuals(sum)
# samples are taking from same patients for both normal and cancer tissue
